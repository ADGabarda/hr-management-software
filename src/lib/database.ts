...
class DatabaseManager {
  ...
  private getData() {
    ...
    return data ? JSON.parse(data) : { users: [], posts: [], nextUserId: 1, nextPostId: 1 };
  }
  ...
  // User operations
  createUser(username: string, password: string, name: string, role: 'admin' | 'user' = 'user'): User {
    const data = this.getData();
    
    // Check if username already exists
    const existingUser = data.users.find((user: User) => user.username === username);
    if (existingUser) {
  ...
  // Post operations
  createPost(userId: number, content: string, image: string | null, scheduledFor: string) {
    const data = this.getData();
  ...
  getPostsByUserId(userId: number): Post[] {
    ...
    return data.posts
      ...
      .sort((a: Post, b: Post) => new Date(b.scheduledFor).getTime() - new Date(a.scheduledFor).getTime());
  }
  ...
  deletePost(id: number, userId: number): boolean {
    const data = this.getData();
    const postIndex = data.posts.findIndex((post: Post) => post.id === id && post.userId === userId);
